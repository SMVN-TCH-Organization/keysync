...
function run_cxitool_and_parse() {
    ...
    num_lines=${#cxitool_array[@]}
    log "Number of lines in cxitool_array: $num_lines"
    ...
    for ((i=0; i<num_lines; i++)); do
        ...
        #log "Index: $idx, Algorithm: $algo, Size: $size, Type: $type, Group: $group, Name: $name, Spec: $spec"

        # Run the cxitool command with the extracted name and group
        result=$(cxitool Dev=$sourceip LogonPass=USR_0000,$pin Group=$group Name=$name keyinfo)
        ...
        #echo "Keyinfo $((i+1)):"
        #echo "$result"
        #log "Result for line $((i+1)): $result"
    done

    # Optionally, log all results at the end
    echo "All keypair details:"
    for ((i=0; i<num_lines; i++)); do
        echo "Keyinfo $((i+1)): ${results_array[i]}"
    done
}
